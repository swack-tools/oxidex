...
- Group: `FoundTag` with no active IFD context defaults family-1 group to `File` — verified empirically: `exiftool -G1 -s -PreviewImage SamsungDigimaxA40.jpg` → `[File]  PreviewImage : (Binary data 36864 bytes, use -b option to extract)`.
  - **CORRECTION (found during final whole-branch review):** `SamsungDigimaxA40.jpg` is NOT actually valid ground truth for this task's APP2/APP3/APP4 mechanism. Confirmed via `exiftool -v2 /tmp/oxidex-exiftool-cache/combined-samples/Samsung/SamsungDigimaxA40.jpg` that the file contains no APP2/APP3/APP4/APP5 segment at all — only APP0/APP1/DQT/SOF/DHT. Its `[File] PreviewImage` is produced by an unrelated `MakerNotes.pm` unknown-maker-note code path, not the direct-dump mechanism implemented by this task. The group conclusion (`File`) is still correct, but the citation is not — the real verified ground truth for the APP2/APP3/APP4 mechanism (and for this task's actual implementation and tests) is `GoProHERO.jpg`, a 6-segment APP2/APP3/APP4 accumulation case. Left in place per this file's convention of correcting in place rather than silently deleting.
...
- **Spec coverage:** Task 1 covers the IFD2 bucket (7 files). Task 2 covers the Samsung/HP/BenQ/GoPro/Rollei direct-dump bucket (50 files, the largest). Task 3 covers Sony (48 files, second largest). Task 4 covers the CR2/Canon and DNG remainder (Canon 3 + any DNG files in "others"). Task 5 covers Casio (3) + Minolta (3) + any Olympus files folded into "others". ~~Together these account for all 118 measured files; any residual "others" not matching one of these five mechanisms should be re-measured with the tag-comparison harness after Tasks 1-5 land rather than assumed closed.~~
  - **CORRECTION (found during final whole-branch review):** the "accounts for all 118" claim above is false. A post-landing corpus sweep measured, of 185 Samsung files where real ExifTool emits `PreviewImage`, only 96 now match oxidex's output — 89 are still missing. Those 89 route through two mechanisms this plan never enumerated or implemented: `MakerNoteSamsung1b` (STMN format — e.g. `SamsungNV10.jpg`, `SamsungWB500.jpg`) and `MakerNotePentax` (Samsung bodies that reuse the Pentax MakerNote format — e.g. `SamsungGX20.jpg`). The same sweep also newly found `SonyDSLR-A100.jpg` still missing, and 5 Olympus OM-System-era bodies still missing because oxidex's Olympus MakerNote header-detection does not recognize the `"OM SYSTEM\0"` header at all — a whole-parser gap, not specific to `PreviewImage`. These three items (Samsung STMN, Samsung-via-Pentax-MakerNotes, Olympus OM-System header gap) are known follow-up work, not covered by Tasks 1-5.
