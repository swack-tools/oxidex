...
- `PrimaryAFPoint`'s direct-lookup tables reuse the *same* per-schema table as `AFPointsUsed` (`AF_POINTS_51`/`AF_POINTS_39`/`AF_POINTS_153`), with `0 → "(none)"` and a `1 → "{center} (Center)"` override — bit-number `1` is documented as the center point in every one of these tables (Nikon.pm comments: `1 => 'C6'` for 51/39-point, `1 => 'E9'` for 153-point). For schema `2` (11-point), `PrimaryAFPoint` uses its own direct 1..11 → name table (Nikon.pm:4517-4528), which is *not* `AF_POINTS_11` (see correction below), with no "(Center)" suffix (11-point mode has no single distinguished center per ExifTool's source).

  **Correction found during Task 4 implementation:** `AF_POINTS_11` (extracted in Task 1/2 from the module-level `%afPoints11` hash, Nikon.pm:1441-1457) is the wrong source for this offset-8 case. `%afPoints11` is a *different* table ExifTool uses for the unrelated `AFPointsInFocus` tag (out of scope for this plan); its `BITMASK` ordering diverges from the inline `BITMASK` literal actually used at offset 8 for `AFPointsUsed`/`PrimaryAFPoint` starting at bit 4 (`%afPoints11` has `4 => 'Mid-right'`; the offset-8 literal has `4 => 'Upper-left'`, Nikon.pm:4258-4270 / 4396-4408 / 4517-4528). Using `AF_POINTS_11` here would have silently mislabeled AF points on real D90/D3000-D3300/D5000/D5100/D3400/D3500 files. The correct fix is a second, locally-defined table matching the offset-8 literal's actual ordering (`0 => 'Center', 1 => 'Top', 2 => 'Bottom', 3 => 'Mid-left', 4 => 'Upper-left', 5 => 'Lower-left', 6 => 'Far Left', 7 => 'Mid-right', 8 => 'Upper-right', 9 => 'Lower-right', 10 => 'Far Right'`) — this ordering, shifted +1, is also exactly what `PrimaryAFPoint`'s own direct 1..11 table uses, confirming the two offset-8 tags share one correct table that is simply not `AF_POINTS_11`. `AF_POINTS_11` itself remains correctly transcribed for its actual source (`%afPoints11`) but has no consumer in this plan's scope (`AFPointsInFocus` is explicitly out of scope) — flag this to the task reviewer rather than deleting it, since removing a Task-2-committed const is outside Task 4's scope.
