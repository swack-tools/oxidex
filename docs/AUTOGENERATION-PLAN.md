...
| Sony focus-table pilot | Published at `1a63822e`; complete paired corpus has zero changed per-file discrepancy records. Follow-up removal of 17 duplicate entries is integrated and passes lint/workspace/build; its final corpus check remains. | The shared path preserves current output across all 4,238 files before retirement. The final removal still needs its own evidence and merge. |
...
| Sony raw-ID correctness repair | Local and Linux paired 4,238-file runs complete; corrected i7 workspace, documentation, table and JPEG matrix stages pass; final Linux per-file reconciliation being recorded; not landed | Correct rows rise from 468,002 to 468,012. The local pair identifies ten ImageNumber gains with no other changed residuals. This is correctness progress. The original failed gate remains recorded. |
