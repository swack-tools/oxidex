...
| Sony enciphered recovery | Producer and independent verifier preserved; not landed | The draft still has a Sony-specific translation layer. Its review remains useful, but it is not the architecture target. |
...
| Sony raw-ID correctness repair | Paired 4,238-file comparison complete; landing gate still pending | Correct rows rise from 468,002 to 468,012. Ten files gain ImageNumber; no new missing, wrong-value or extra facts. This is correctness progress. |
...
## The progress report after each landed batch
