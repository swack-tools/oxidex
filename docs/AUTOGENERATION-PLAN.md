...
| Sony focus-table pilot | Published at `1a63822e`; complete paired corpus has zero changed per-file discrepancy records; corrected full workspace passes; duplicate retirement remains | The shared path preserves current output across all 4,238 files. It is not yet a merged migration. |
...
| Sony raw-ID correctness repair | Paired 4,238-file comparison complete; corrected i7 workspace test invocation passes; remaining gate stages pending; not landed | Correct rows rise from 468,002 to 468,012. Ten files gain ImageNumber; no new missing, wrong-value or extra facts. This is correctness progress. The original failed gate remains recorded. |
