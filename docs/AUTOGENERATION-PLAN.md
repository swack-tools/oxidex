...
| Sony focus-table pilot | Published in ready-for-review PR #746, including removal of 17 duplicate entries. Hosted CI passes at `c3d0acd7`. Final Linux checks and the 4,238-file pair pass at `92dac917`: ten raw-ID fixes and no other per-file changes. | The final removal is implemented, reviewed and corpus-validated. The evidence and mandatory completeness-check follow-up still need final CI and merge. |
| Shared binary strings | Published at `a93f1e4d`; local lint, 5,916 workspace tests including doctests, C-header and formatting checks pass. All 563 Python tool tests pass; the full corpus pair is next. | This adds shared support for raw string bytes and native remainder lengths. No additional parser uses it yet; no Canon manual reader has been removed. |
...
| Sony raw-ID correctness repair | Local and Linux paired 4,238-file runs complete; the final combined gate now includes per-file JSON evidence. Linux code, native-table and JPEG matrix stages pass; not landed. | Correct rows rise from 468,002 to 468,012. Both local and final Linux pairs identify ten ImageNumber gains with no other changed residuals. The earlier failed attempts and aggregate-only pair remain recorded. |
...
Finish and merge the **Sony focus-table pilot**, reconciling its final per-file
comparison and publishing the exact manual rules removed. Preserve failed gate
attempts and their corrections alongside the completed evidence.
