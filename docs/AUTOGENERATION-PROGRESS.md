...
attempt, its formatting repair and the remaining acceptance work. The active
[production migration](reference/afinfo2-production-plan.md) connects the
authenticated parent edges, verifies complete output and removes the AFInfo2
manual reader in one accepted batch. A parent connection alone retires zero
