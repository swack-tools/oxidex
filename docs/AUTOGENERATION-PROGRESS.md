...
## Input-normalization source checkpoint

On `codex/sanitize-integration-20260914`, the official dumper now records the
final Sanitize body, four direct dependencies and the SetWarning callback
reference. A source-mechanism parser extracts the version guards while retaining
unresolved branch evidence. It validates each resolved dependency's provenance;
an independent review caught and rejected an earlier malformed-fact acceptance.
This parser does not admit a runtime writer or a caller-supplied Encode flag.

Fresh `dump_tables.pl` captures of the merged base and this candidate, using
ExifTool 13.59 and Perl 5.38.2, have identical read projections for all 153
modules and identical reader contracts. The captured native Sanitize mechanism
compiles. All 57 focused source/helper tests pass with no skips, including
actual native scalar comparisons and source-change capture tests. Evidence is
under `shared-pilot/write-upgrade-integration-20260913/sanitize-integration-20260914/`
relative to the continuation evidence root. Full regeneration and final branch
gates remain pending; this is an implementation checkpoint, not a new release.

Next, join a pristine interpreter encoding witness to the actual final dumper
state and validate the emitted Rust sanitization. Then complete the inverse
conversion, charset/count and physical file-operation stages described in the
[writer pipeline](reference/writer-input-pipeline.md). The new module in the
native probe worker's tree is not yet integrated or accepted as runtime proof.
