...
This checkpoint makes the native serial layouts executable by one shared
reader. It has no production caller. The next delivery replaces Real AudioV4's
manual field sequence after carrier and output checks pass.
...
No new project autogenerated percentage or manual-reader retirement is claimed
by this checkpoint.
