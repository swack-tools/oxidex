...
| Metric | Value |
|--------|-------|
| Total Tags | 32,000+ (tag definitions; see [Tag Coverage](/reference/tag-coverage-analysis) for the measured extraction score) |
| Modules Parsed | 140+ |
| Lookup Time | O(1) |
...
As of Step 30 of the tag-machinery overhaul (see `AGENTS.md`, "Tag knowledge
is not tag coverage"), the registry is generated from `dump_tables.pl`'s dump
of ExifTool's real Perl tag tables — the same JSON `tools/exiftool-tables/
regen.sh` already produces for the binary-table generator — not from
`exiftool -f -listx` (the documentation view). Tags are regenerated as part of
that script, or explicitly via `gen_tag_registry`:

```bash
cargo run --release --bin gen_tag_registry -- target/exiftool-src/tables-<version>.json
```

```
1. Dump     → tools/exiftool-tables/dump_tables.pl walks ExifTool's live Perl
              symbol table (not -listx) and emits every table's real fields
              as JSON: Format, Writable, SubDirectory, Description, and so on
2. Parse    → src/tag_sync/mod.rs::tag_records_from_dump_json reads it,
              resolving WRITABLE inheritance, the declared type, and any
              missing description (ExifTool's own MakeDescription algorithm)
              by hand, and merging each module's own %Composite sub-table
              into the single "Composite" table -listx reports
...
Because `dump_tables.pl` reads the real table structures rather than
`-listx`'s filtered documentation view, this also carries every tag `-listx`
omits for having no printable value — `ExifOffset`, `GPSInfo`, `InteropOffset`
and the other `SubDirectory` pointers oxidex needs in order to find anything
at all — with no separate carry-forward step required to keep them.

`-listx` and `src/tag_sync::parse_listx` still exist, but only as the
independent oracle `tests/tag_registry_invariants.rs` grades the generated
registry against (catching PrintConv display values that leak in as tags);
they no longer generate it.
...
To regenerate the tag database, run `tools/exiftool-tables/regen.sh` (which
also refreshes `binary_tables.rs`, the filetype tables, Composite definitions
and FITS keywords from the same pinned ExifTool source), or invoke
`gen_tag_registry` directly against an existing `dump_tables.pl` JSON dump —
see [Tag Generation Pipeline](#tag-generation-pipeline). Either overwrites
`oxidex-tags-*/src/*_tags.yaml` directly — review the resulting `git diff`
before committing.
...
## XML Parser Features (oracle only)

`parse_listx` in `src/tag_sync/mod.rs` no longer generates the registry (see
above), but remains as the reader `tests/tag_registry_invariants.rs` uses to
grade it against a live `exiftool -f -listx`. It handles:
