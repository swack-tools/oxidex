...
5. Keep the proven primitives byte-identical where they fit and re-home them: `fleetlib.Hub` CAS,
   ...
   `intent.py`/`ledger.py` checks, `train.py` batching/bisect/exact-set push, `gate.sh` v8
   (v7 plus the 1800 s fleet-tests budget, 2026-08-22).
...
| C6 | `keel-server` (`tools/fleet/keel/server.py`; `http.server.ThreadingHTTPServer`, HTTP/1.1 keep-alive, chunked SSE) | py ≥3.10 stdlib | the current lease holder (i7 primary — the ONLY server candidate since the ryzen was removed 2026-08-22; macOS laptops are never candidates) | NONE of truth. Local only: `~/.keel/events.db` (sqlite, lossy ring), `~/.keel/code.git` mirror, `~/.keel/logs/` | runners fall back (C5); election re-hosts (§3.4) |
...
| C8 | `gate.sh` GATE_VERSION 8 (v7 + fleet-tests budget 1800 s), stage order unchanged | bash | runners | `~/gatelogs/gate-<tag>.{log,verdict,json}` | exit 0/1/7/9 as today (`gate.sh` L355-391) |
...
1. `register` on start and every reconnect: `doctor.py`'s measurements (`platform_id`, `rustc_id`
   ...
   `gate_version` 8; `tools_tree_sha`; `clis:[claude,codex]` whose auth probe passed; `has_oracle`,
...
|---|---|---|---|---|
| **Server host dies** (written when the ryzen was a candidate — removed 2026-08-22; today this is the i7, and with it the only candidate, so re-election waits for it to return) | runners' long-polls fail; after 3 failures `FallbackHub` renews leases and stores verdicts directly on the state repo (same CAS, same refs); logs spool; eligible runners poll the server lease and elect (§3.4) | no running gate/agent is killed; no lease lapses while GitHub is reachable; verdicts land; tip cannot move (deploy key is only used by the train thread under a live train claim) | ≤ 5 min of events, in-memory log rings, new assignments for ≈ TTL 120 s + rank×60 s + settle 180 s ≈ 4–6 min | automatic; i7 `autonomous_when_serverless` starts gates meanwhile |
...
| **Stale ryzen hub copy returns** (the ryzen was removed from the fleet 2026-08-22; this row remains as the recipe should its disk ever resurface) | import only with a non-forced `git push state 'refs/fleet/*:refs/fleet/*'` (creates where absent; conflicts refused); verdict `conflict`s reported, never merged; its old `fleetd` exits 2 (shim) | no overwrite | — | human, once |
...
| `gate.sh` (641) | **keep**, GATE_VERSION 8 (the 2026-08-22 fleet-tests-budget bump; behaviour otherwise v7's): L243 and L373 hard-coded `HUB_URL` defaults → `FLEET_CODE_URL` (required, fail loud); `verdict.py … --server-url "$KEEL_SERVER_URL"`; `EXIFTOOL`/DOCX paths (L223, L441) from `EXIFTOOL_CACHE_DIR` with the `/tmp` default kept on Linux and `~/oxidex-cache` on macOS; nothing in stage order, `classify_failure` (L321-352), `write_json` (L254-290), flake handling or exit codes changes — `test_gate_script.TestGateVersionMatchesFile` enforces | proven gate executable |
