...
# Top CPU consumers, highest first. BSD/macOS ps sorts by CPU with -r; procps
# (Linux) has no -r and sorts with --sort=-pcpu instead.
top_ps() {
    if [ "$(uname -s)" = Darwin ]; then ps -Ao pcpu,args -r; else ps -Ao pcpu,args --sort=-pcpu; fi
}
...
snap() {
    { echo "[$1] $(uptime)"; top_ps | sed -n '1,6p' | sed "s/^/[$1]   /" | cut -c1-140; } | tee -a "$RUN_LOG"
}
...
table() { # json
    jq -r --arg tmp "$TEMP_DIR/" --arg root "$PROJECT_ROOT/" '
      ...
      (.results[] | "| <code>" + (.command | gsub("/tmp/oxidex-perl538-build-[^ ]+/perl5.38.2 -I[^ ]+ [^ ]+/exiftool"; "exiftool") | gsub("[^ ]+/target/release/oxidex"; "oxidex") | gsub("/tmp/oxidex-exiftool-cache/exiftool/t/images/"; "") | gsub($tmp; "") | gsub($root; "") | gsub("&"; "&amp;") | gsub("<"; "&lt;") | gsub(">"; "&gt;") | gsub("\\|"; "&#124;") | gsub("`"; "&#96;") | gsub("\\r?\\n"; "&#10;")) + "</code> | " + (.median | ms | tostring) + " ms | " + (.min | ms | tostring) + " ms | " + (.mean | ms | tostring) + " ± " + (.stddev | ms | tostring) + " | " + (.max | ms | tostring) + " ms | " + (.times | length | tostring) + " |")' "$1"
}
...
- **Environment**: ${BENCH_ENVIRONMENT:-local run, not CI (BENCH_ENVIRONMENT unset)}
...
MD
    ...
    jq -n \
        ...
        --arg et "$EXIFTOOL_PROVENANCE" --arg etv "$EXIFTOOL_VERSION" --arg machine "$MACHINE" --arg env "${BENCH_ENVIRONMENT:-local run, not CI}" --arg load "$LOADAVG" --arg top "$TOP_CPU" \
        ...
        '{instrument: {commit: $commit, describe: $describe, dirty: ($dirty == "1"), oxidex: $bin, oxidex_sha256: $sha,
                       ...
                       machine: $machine, environment: $env, loadavg_at_preflight: $load, top_cpu_at_preflight: $top},
