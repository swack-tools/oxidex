...
fn process_tiff_ifd_tags<'a>(
    ...
    for (tag_id, field_type, value_count, raw_bytes) in tags {
        ...
        if *tag_id == 0x83BB && !bytes.is_empty() {
            // Decoded by the shared IIM walk, so the IPTC in a TIFF reads
            // identically to the same bytes in a JPEG APP13 segment. This
            // branch used to aggregate Keywords but not SupplementalCategories,
            // and dropped the envelope record entirely.
            crate::parsers::jpeg::iptc_parser::insert_iptc_records(bytes, metadata);
