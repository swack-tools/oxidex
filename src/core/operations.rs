...
pub fn write_metadata(path: &Path, metadata: &MetadataMap) -> Result<()> {
    ...
    // PHASE 4: SERIALIZE WITH APPROPRIATE WRITER AND WRITE TO DISK
    //
    // JPEG serializes to a byte buffer that is then written atomically here.
    // PNG/PDF/TIFF writers own their full read-modify-write cycle (including the
    // atomic write to `path`), so they are routed directly through their
    // dedicated entrypoints.
    match format {
        FileFormat::JPEG => {
            // Use JPEG writer to serialize metadata, then write atomically.
