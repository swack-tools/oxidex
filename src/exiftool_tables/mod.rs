...
/// sibling for [`ALL_IFD_TABLES`]. The two name spaces are disjoint by
/// construction (`codegen.py` sorts every table into exactly one of them by
/// `PROCESS_PROC`), so a `SubDirectory` edge resolves by asking this first
/// and [`find_table`] second; a `Some` from both would be a generator bug.
///
