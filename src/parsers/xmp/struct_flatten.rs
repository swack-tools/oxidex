...
fn close_frame(stack: &mut Vec<Frame>, collected: &mut Vec<(String, Vec<String>)>) {
    ...
    // before the generic leaf handling drops the `rdf:type` frame (ported
    // from origin/main 47037a04).
...
fn google_device_container_type_tag(stack: &[Frame], type_uri: &str) -> Option<String> {
    ...
    for frame in stack.iter().rev() {
        ...
        match (frame.part.as_deref(), type_name) {
            (Some("Cameras"), "Camera") => return Some(format!("{}:Cameras", frame.group)),
            (Some("Profiles"), "Profile") => return Some(format!("{}:Profiles", frame.group)),
            _ => {}
        }
    }
    None
   ...
   <rdf:Seq>
    <rdf:li rdf:parseType="Resource">
     ...
     <rdf:type>http://ns.google.com/photos/dd/1.0/device/:Camera</rdf:type>
        ...
        assert_eq!(
            value_of(&tags, "XMP-Device:Cameras").as_deref(),
  ...
  <Device:Profiles>
   <rdf:Seq><rdf:li rdf:parseType="Resource">
    <rdf:type>http://ns.google.com/photos/dd/1.0/device/:Profile</rdf:type>
        ...
        let tags = extract_flattened_struct_fields(XMP).unwrap();
        assert_eq!(
            value_of(&tags, "XMP-Device:Profiles").as_deref(),
