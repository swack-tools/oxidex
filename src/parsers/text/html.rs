...
/// `Image::ExifTool::XMP::FixUTF8`, which the `exiftool` writer applies to
/// every value on its way out: each byte that does not begin a valid UTF-8
/// sequence is replaced by a single `?`.
///
/// This matters because ExifTool does *not* recode when no charset is
/// declared, so a Latin-1 byte in such a file reaches the writer raw and is
/// reported as `caf?`. Rust's own lossy conversion collapses a whole invalid
/// run into one U+FFFD instead, which is a different string.
fn fix_utf8(data: &[u8]) -> String {
    ...
    while i < data.len() {
        ...
        if (0xC2..0xF8).contains(&lead) {
            ...
            let tail = data.get(i + 1..i + 1 + n);
            if let Some(tail) = tail
                && tail.iter().all(|&b| (0x80..=0xBF).contains(&b))
            {
                let malformed = match n {
                    1 => false,
                    2 => {
                ...
                if !malformed {
                    out.extend_from_slice(&data[i..i + 1 + n]);
                    i += 1 + n;
                    continue;
                }
            }
        }
        // Only the offending byte is replaced; scanning resumes right after
        // it, so each byte of a malformed sequence becomes its own '?'.
        out.push(b'?');
        i += 1;
    }
...
/// ```
fn attr_content(attrs: &[u8]) -> Option<&[u8]> {
    // The two patterns are tried one after the other over the whole attribute
    // string, not interleaved per occurrence, because that is what `or` between
    // two full matches means in Perl.
    content_quoted(attrs).or_else(|| content_unquoted(attrs))
}

/// The value start for one `content` occurrence: just past `content\s*=\s*`.
fn content_positions(attrs: &[u8]) -> impl Iterator<Item = usize> + '_ {
    let mut from = 0;
    std::iter::from_fn(move || {
        loop {
            let index = find_ignore_case(&attrs[from..], b"content")?;
            let at = from + index;
            from = at + 1;
...
/// `/\bcontent\s*=\s*(['"])(.*?)\1/si`
fn content_quoted(attrs: &[u8]) -> Option<&[u8]> {
    content_positions(attrs).find_map(|i| {
        let &quote = attrs.get(i)?;
        if quote != b'\'' && quote != b'"' {
            return None;
        }
        let close = attrs[i + 1..].iter().position(|&b| b == quote)?;
        Some(&attrs[i + 1..i + 1 + close])
    })
}

/// `/\bcontent\s*=\s*(['"]?)([\w:.-]+)/si`
fn content_unquoted(attrs: &[u8]) -> Option<&[u8]> {
    content_positions(attrs).find_map(|i| {
        let mut j = i;
...
/// The normalisation `AddTagToTable` applies to every dynamically added tag,
/// after the module's own name mangling (ExifTool.pm:9256-9265):
///
/// ```text
...
/// ```
///
/// This is why an unknown `dc.`-style namespace reports
/// `UnknownNsWeird_thing` and not the `unknownNsWeird_thing` that HTML.pm's
/// own substitution alone produces.
fn normalize_added_tag_name(name: &str) -> String {
    ...
    if cleaned.len() < 2 || !cleaned.starts_with(|c: char| c.is_ascii_alphabetic()) {
        cleaned.insert_str(0, "Tag");
    }
    cleaned
}
...
mod tests {
    ...
    /// With no declared charset ExifTool does not recode, so its writer's
    /// `FixUTF8` is what the value ends up going through. It replaces each bad
    /// byte with one '?', where Rust's lossy conversion would collapse a run
    /// into a single U+FFFD.
    #[test]
    fn undeclared_charset_falls_back_to_fixutf8() {
        assert_eq!(fix_utf8(b"caf\xe9"), "caf?");
        assert_eq!(fix_utf8(b"a\xff\xfeb"), "a??b");
        // Valid UTF-8 passes through untouched.
        assert_eq!(fix_utf8("café €".as_bytes()), "café €");
        // An overlong encoding is malformed: the lead byte and each stranded
        // continuation byte become their own '?'.
        assert_eq!(fix_utf8(b"\xe0\x80\x80"), "???");
        assert_eq!(decode(b"caf\xe9", None), "caf?");
        assert_eq!(decode(b"caf\xe9", Some(Charset::Latin)), "café");
    }
    ...
    /// A dynamically added tag goes through the module's mangling *and*
    /// `AddTagToTable`'s normalisation. Missing the second step reported
    /// `unknownNsWeird_thing` where ExifTool reports `UnknownNsWeird_thing`.
    #[test]
    fn added_tag_names_get_addtagtotable_normalisation() {
        assert_eq!(
            normalize_added_tag_name(&meta_tag_name("unknown-ns:weird_thing")),
            "UnknownNsWeird_thing"
        );
        assert_eq!(normalize_added_tag_name(&xml_tag_name("test1")), "Test1");
        // Fewer than two characters, or not starting with a letter, gets the
        // "Tag" prefix.
        // ucfirst runs before the length test, so the prefixed name keeps the
        // capital: `ucfirst "x"` is "X", then "Tag" . "X".
        assert_eq!(normalize_added_tag_name("x"), "TagX");
        assert_eq!(normalize_added_tag_name("9lives"), "Tag9lives");
        assert_eq!(normalize_added_tag_name(":::"), "Tag");
    }
