//! GeoTIFF key names and PrintConv maps, transcribed EXACTLY from the
//! pinned ExifTool 13.59 `lib/Image/ExifTool/GeoTiff.pm`.
//!
//! Generated by `tools/exiftool-tables/gen_geotiff_printconv.py` -- DO NOT
//! EDIT BY HAND; rerun the generator (it dumps the live
//! `%Image::ExifTool::GeoTiff::Main` hash through the pinned perl, so the
//! values here are what ExifTool itself resolves, including Perl's
//! last-assignment-wins on GeoTiff.pm's duplicated key 2177).
//!
//! `GeoTiff.pm` declares no `OTHER`, `BITMASK`, or code PrintConv, so these
//! plain maps are the complete conversion story; a lookup miss prints as
//! `Unknown (N)` exactly like ExifTool.pm's HASH PrintConv fallback
//! (ExifTool.pm line 3633, no PrintHex on any GeoTiff tag).

/// GTModelType PrintConv, inside Main (lines 102-2128)
pub static GT_MODEL_TYPE: &[(u16, &str)] = &[
    (1, "Projected"),
    (2, "Geographic"),
    (3, "Geocentric"),
    (32767, "User Defined"),
...
/// GTRasterType PrintConv, inside Main (lines 102-2128)
pub static GT_RASTER_TYPE: &[(u16, &str)] = &[
    (1, "Pixel Is Area"),
    (2, "Pixel Is Point"),
    (32767, "User Defined"),
];

/// GeographicType PrintConv (# epsg_gcs), inside Main (lines 102-2128)
pub static EPSG_GCS: &[(u16, &str)] = &[
    (4001, "Airy 1830"),
...
/// GeogGeodeticDatum PrintConv (# epsg_datum), inside Main (lines 102-2128)
pub static EPSG_DATUM: &[(u16, &str)] = &[
    (6001, "Airy 1830"),
...
/// GeogPrimeMeridian PrintConv (# epsg_pm), inside Main (lines 102-2128)
pub static EPSG_PM: &[(u16, &str)] = &[
    (8901, "Greenwich"),
    (8902, "Lisbon"),
    (8903, "Paris"),
    (8904, "Bogota"),
...
/// %epsg_units (lines 32-57)
pub static EPSG_UNITS: &[(u16, &str)] = &[
    (9001, "Linear Meter"),
    (9002, "Linear Foot"),
...
/// GeogEllipsoid PrintConv (# epsg_ellipse), inside Main (lines 102-2128)
pub static EPSG_ELLIPSE: &[(u16, &str)] = &[
    (7001, "Airy 1830"),
...
/// ProjectedCSType PrintConv (# epsg_pcs), inside Main (lines 102-2128)
pub static EPSG_PCS: &[(u16, &str)] = &[
...
/// Projection PrintConv (# epsg_proj), inside Main (lines 102-2128)
pub static EPSG_PROJ: &[(u16, &str)] = &[
...
/// ProjCoordTrans PrintConv (# geo_ctrans), inside Main (lines 102-2128)
pub static GEO_CTRANS: &[(u16, &str)] = &[
...
/// %epsg_vertcs (lines 59-100)
pub static EPSG_VERTCS: &[(u16, &str)] = &[
    (0, "Undefined"),
...
/// ChartFormat PrintConv, inside Main (lines 102-2128)
pub static CHART_FORMAT: &[(u16, &str)] = &[
    (47500, "General"),
    (47501, "Coastal"),
    (47502, "Harbor"),
...
/// ChartSoundingDatum PrintConv, inside Main (lines 102-2128)
pub static CHART_SOUNDING_DATUM: &[(u16, &str)] = &[
...
/// Key ID -> tag name, from %Main (lines 102-2128). Keys absent here are
/// skipped entirely, mirroring ProcessGeoTiff's `GetTagInfo(...) or next`.
pub static GEOKEY_NAMES: &[(u16, &str)] = &[
    (1, "GeoTiffVersion"),
    (1024, "GTModelType"),
    (1025, "GTRasterType"),
    (1026, "GTCitation"),
    ...
    (47008, "ChartSoundingDatum"),
    (47009, "ChartDepthUnits"),
    (47010, "ChartMagVar"),
...
/// The PrintConv map for a key, or None for keys printed as-is.
pub fn geokey_print_conv(key_id: u16) -> Option<&'static [(u16, &'static str)]> {
    match key_id {
        1024 => Some(GT_MODEL_TYPE),
        1025 => Some(GT_RASTER_TYPE),
        2048 => Some(EPSG_GCS),
        2050 => Some(EPSG_DATUM),
        2051 => Some(EPSG_PM),
        2052 | 2054 | 2060 | 3076 | 4099 | 47009 => Some(EPSG_UNITS),
        2056 => Some(EPSG_ELLIPSE),
        3072 => Some(EPSG_PCS),
        3074 => Some(EPSG_PROJ),
        3075 => Some(GEO_CTRANS),
        4096 | 4098 => Some(EPSG_VERTCS),
        47001 => Some(CHART_FORMAT),
        47008 => Some(CHART_SOUNDING_DATUM),
        _ => None,
    }
}

/// The ExifTool tag name for a GeoTIFF key ID, or None if ExifTool's Main
/// table has no entry (ProcessGeoTiff skips such keys without -u).
pub fn geokey_name(key_id: u16) -> Option<&'static str> {
    GEOKEY_NAMES
        .binary_search_by_key(&key_id, |&(k, _)| k)
        .ok()
        .map(|i| GEOKEY_NAMES[i].1)
}

/// Look up `value` in a PrintConv map (maps are sorted by key).
pub fn print_conv_lookup(map: &'static [(u16, &str)], value: u16) -> Option<&'static str> {
    map.binary_search_by_key(&value, |&(k, _)| k)
        .ok()
        .map(|i| map[i].1)
}
