...
mod tests {
    ...
    /// Step 22 regression: PNG's `iCCP` chunk must land under `ICC_Profile:`
    /// with each tag's real family-1 group attached from table provenance,
    /// not the internal `Profile:` prefix this format used to leak (see
    /// `src/parsers/icc/mod.rs`'s module doc comment) with no group split at
    /// all -- the only other format that ever rewrote `Profile:` to
    /// `ICC_Profile:` was JPEG's own `normalize_metadata_map` post-pass,
    /// which PNG's own parse path never called.
    ///
    /// The fixture (`tests/fixtures/png/icc_cicp/step22-hdr-cicp.png`,
    /// generated by this session, not from the pinned corpus -- no
    /// `cicp`-bearing ICC profile exists anywhere in
    /// `/tmp/oxidex-exiftool-cache/combined-samples`, confirmed by a
    /// full-corpus byte search) is a 1x1 truecolor PNG whose `iCCP` chunk
    /// carries a 220-byte ICC v4.3 profile: a header, a `desc` tag, and a
    /// `cicp` (HDR coding-independent code points) tag. Verified byte-for-
    /// byte against the pinned oracle before pinning here:
    ///
    /// ```text
    /// /usr/bin/perl5.34 -I/tmp/oxidex-exiftool-cache/exiftool/lib \
    ///   /tmp/oxidex-exiftool-cache/exiftool/exiftool -a -G1 -s step22-hdr-cicp.png
    ///
    /// [ICC-header]  ProfileCMMType           : Apple Computer Inc.
    /// [ICC-header]  ColorSpaceData           : RGB
    /// [ICC_Profile] ProfileDescription       : OxiDex Step22 HDR Test Profile
    /// [ICC-cicp]    ColorPrimaries           : BT.2020, BT.2100
    /// [ICC-cicp]    TransferCharacteristics  : SMPTE ST 2084, ITU BT.2100 PQ
    /// [ICC-cicp]    MatrixCoefficients       : BT.2020 non-constant luminance, BT.2100 YCbCr
    /// [ICC-cicp]    VideoFullRangeFlag       : Full
    /// ```
    #[test]
    fn iccp_chunk_splits_family1_groups_from_table_provenance() {
        let bytes = include_bytes!("../../../tests/fixtures/png/icc_cicp/step22-hdr-cicp.png");
        let reader = TestReader::from_slice(bytes);
        let metadata = parse_png_metadata(&reader).expect("parse_png_metadata");

        let group1_of = |key: &str| -> String {
            metadata
                .all_occurrences()
                .find(|(k, _)| k == key)
                .unwrap_or_else(|| panic!("missing occurrence for {key}"))
                .1
                .group1
                .to_string()
        };

        assert_eq!(group1_of("ICC_Profile:ProfileCMMType"), "ICC-header");
        assert_eq!(group1_of("ICC_Profile:ColorSpaceData"), "ICC-header");
        assert_eq!(
            group1_of("ICC_Profile:ProfileDescription"),
            "",
            "an un-grouped %Main tag carries no family-1 override"
        );
        assert_eq!(group1_of("ICC_Profile:ColorPrimaries"), "ICC-cicp");
        assert_eq!(group1_of("ICC_Profile:TransferCharacteristics"), "ICC-cicp");
        assert_eq!(group1_of("ICC_Profile:MatrixCoefficients"), "ICC-cicp");
        assert_eq!(group1_of("ICC_Profile:VideoFullRangeFlag"), "ICC-cicp");

        assert_eq!(
            metadata.get_string("ICC_Profile:ColorPrimaries"),
            Some("BT.2020, BT.2100")
        );
        assert_eq!(
            metadata.get_string("ICC_Profile:TransferCharacteristics"),
            Some("SMPTE ST 2084, ITU BT.2100 PQ")
        );
        assert_eq!(
            metadata.get_string("ICC_Profile:MatrixCoefficients"),
            Some("BT.2020 non-constant luminance, BT.2100 YCbCr")
        );
        assert_eq!(
            metadata.get_string("ICC_Profile:VideoFullRangeFlag"),
            Some("Full")
        );
        assert_eq!(
            metadata.get_string("ICC_Profile:ProfileDescription"),
            Some("OxiDex Step22 HDR Test Profile")
        );

        // The pre-Step-22 internal `Profile:` prefix must be entirely gone.
        assert!(
            !metadata.keys().any(|k| k.starts_with("Profile:")),
            "no tag should remain under the retired internal `Profile:` prefix"
        );
    }
