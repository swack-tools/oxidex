...
pub fn parse_pe_metadata(reader: &dyn FileReader) -> Result<MetadataMap> {
    ...
    crate::core::metadata_map::file_rows(|| -> Result<MetadataMap> {
        ...
        let (_, dos_header) = parse_dos_header(dos_data).map_err(|e| {
            ExifToolError::parse_error(format!("Failed to parse DOS header: {:?}", e))
        })?;
        ...
        let (remaining, coff_header) = parse_coff_header(pe_data).map_err(|e| {
            ExifToolError::parse_error(format!("Failed to parse COFF header: {:?}", e))
        })?;
