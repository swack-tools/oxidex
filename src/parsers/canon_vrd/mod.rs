...
/// literals, so there is one source for them. `add_identity_tags` fills these
/// in for every read, but only when the filename carries a recognised
/// extension; these two formats are identified by signature, so naming them
/// here keeps the answer tied to the bytes rather than the filename.
...
/// Stamps the `File:` identity tags from `filetype`'s generated tables.
