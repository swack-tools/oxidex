//! Sigma/Foveon MakerNote decoder for the JPEG preview inside an X3F.
//!
//! `SigmaRaw.pm`'s Main NOTES say ExifTool runs its ordinary JPEG reader over
//! an X3F's JpgFromRaw image, so the preview's ExifIFD MakerNote (0x927C) is
//! read through `MakerNotes.pm`'s `MakerNoteSigma` entry into
//! `Image::ExifTool::Sigma::Main`. That is where every `[Sigma]` /
//! family-0 `MakerNotes` tag an X3F reports comes from -- 39 of them on
//! SigmaDP2.x3f.
//!
//! # Why this is not the shared MakerNote dispatcher
//!
//! `parsers::tiff::makernotes::sigma` carries a tag registry that disagrees
//! with `Sigma.pm` on most ids from 0x0018 up: it has `LensRange` at 0x000a
//! (Sigma.pm: `LensFocalRange`), `LensType`/`LensID`/`LensModel` at
//! 0x001a-0x001c (Sigma.pm: `PreviewImageStart`/`PreviewImageLength`/
//! `PreviewImageSize`), and invents 0x0020/0x0021/0x0040/0x0041/0x0050/0x0051
//! outright. Dispatching to it would trade missing tags for wrong ones, so
//! this module transcribes `Sigma.pm` directly instead. The tiff-side registry
//! is left untouched: it is shared by every Sigma JPEG and fixing it is a
...
/// Decodes the Sigma MakerNote of an X3F's embedded preview JPEG.
///
/// * `tiff` - the preview's TIFF block (offset 0 == TIFF header), which is
