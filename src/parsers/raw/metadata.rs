        ...
        match parse_ifd(&reader, ifd_offset, byte_order) {
            Ok(tags) => {
                ...
                for (tag_id, field_type, value_count, raw_bytes) in &tags {
                    ...
                    if *tag_id == 0x02BC {
                        if let Ok(xmp_tags) =
                            crate::parsers::xmp::rdf_parser::parse_xmp_prioritized(bytes)
                        {
                            for (tag_name, tag_value, priority) in xmp_tags {
                                crate::parsers::xmp::rdf_parser::insert_xmp_tag(
                                    &mut metadata,
                                    tag_name,
                                    TagValue::new_string(tag_value.into_joined()),
                                    priority,
                                );
                            }
                        }
...
fn parse_cr3(data: &[u8], _format: RawFormat) -> Result<MetadataMap> {
    ...
    if let Some(payload) = find_cr3_uuid_box(data, &UUID_XMP)
        && let Ok(xmp_tags) = crate::parsers::xmp::rdf_parser::parse_xmp_prioritized(payload)
    {
        for (tag_name, tag_value, priority) in xmp_tags {
            crate::parsers::xmp::rdf_parser::insert_xmp_tag(
                &mut metadata,
...
mod rational_array_tests {
    ...
    fn cr2_preview_image_is_published_only_for_jpeg_data() {
        ...
        let metadata =
            parse_raw_metadata(&synthetic_cr2_with_preview(invalid), RawFormat::CanonCR2)
        ...
        let metadata = parse_raw_metadata(&synthetic_cr2_with_preview(jpeg), RawFormat::CanonCR2)
            .expect("valid synthetic CR2");
        assert_eq!(
            metadata.get("EXIF:PreviewImage"),
