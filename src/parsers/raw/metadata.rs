...
fn extract_rw2_embedded_exif_tags(
    ...
    // The preview's IFD1 holds the JPEG thumbnail. Only the three tags
    // ExifTool reports for it that oxidex was missing are taken; IFD1's
    // Compression/Orientation/ResolutionUnit duplicate values already emitted
    // from elsewhere and collapse onto the same family-0 "EXIF" keys.
    extract_rw2_preview_thumbnail(
        &reader,
        tiff_data,
        first_ifd_offset,
        ifd0_tags.len(),
...
/// Emit EXIF ThumbnailOffset/ThumbnailLength/ThumbnailImage from the IFD1 of
/// the RW2's JpgFromRaw preview.
///
/// ThumbnailOffset is stored relative to the preview's own TIFF header;
/// ExifTool reports it as a position in the physical file (11976 for
/// Panasonic.rw2, which is the stored 10428 plus the preview TIFF's 1548-byte
/// offset into the RW2), so `tiff_base_in_file` is added back.
fn extract_rw2_preview_thumbnail(
    reader: &SliceReader<'_>,
    tiff_data: &[u8],
    first_ifd_offset: u64,
    ifd0_entry_count: usize,
    tiff_base_in_file: usize,
    byte_order: ByteOrder,
    metadata: &mut MetadataMap,
) {
    // The "next IFD" pointer trails IFD0's entries.
    let next_ifd_pos = match usize::try_from(first_ifd_offset)
        .ok()
        .and_then(|start| start.checked_add(2 + ifd0_entry_count * 12))
    {
        Some(position) => position,
        None => return,
    };
    let Some(next_ifd_bytes) = tiff_data.get(next_ifd_pos..next_ifd_pos + 4) else {
        return;
    };
    let next_ifd_offset = u64::from(read_u32(next_ifd_bytes, byte_order));
    if next_ifd_offset == 0 {
        return;
    }
    let Ok(ifd1_tags) = parse_ifd(reader, next_ifd_offset, byte_order) else {
        return;
    };
    ...
    for (tag_id, _field_type, _value_count, raw_bytes) in &ifd1_tags {
        ...
        match *tag_id {
            0x0201 if bytes.len() >= 4 => {
                let stored = u64::from(read_u32(bytes, byte_order));
                metadata.insert(
                    "IFD1:ThumbnailOffset".to_string(),
                    TagValue::new_integer((stored + tiff_base_in_file as u64) as i64),
                );
            }
            0x0202 if bytes.len() >= 4 => {
...
fn extract_interop_index(
    ...
    for (tag_id, _field_type, _value_count, raw_bytes) in &interop_tags {
        match *tag_id {
            ...
            // InteropVersion (0x0002): UNDEFINED[4] holding four ASCII
            // digits. Exif.pm gives it no PrintConv, so ExifTool prints the
            // bytes verbatim ("0100").
            0x0002 => {
                let raw = String::from_utf8_lossy(raw_bytes.as_ref())
                    .trim_end_matches('\0')
...
fn parse_x3f_image_section(data: &[u8], metadata: &mut MetadataMap, format: RawFormat) {
    ...
    if (image_type == 2 || image_type == 3) && columns > 0 && rows > 0 {
        ...
        if data.len() > 28 {
            ...
            if let Ok(Some((_tiff_start, tiff_data))) = find_jpeg_exif_tiff(jpeg_data) {
