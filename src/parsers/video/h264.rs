...
mod tests {
    ...
    /// The sequence parameter set NAL carried by PID 0x1011 of the M2TS.mts
    /// sample, copied verbatim from the file, which ExifTool reports as
    /// 1920x1080. It still carries its emulation-prevention bytes (the two
    /// `00 00 03` runs), so this also exercises `remove_emulation_prevention`
    /// on real data rather than a hand-built stream.
    const FIXTURE_SPS: &[u8] = &[
        0x27, 0x64, 0x00, 0x28, 0xad, 0x00, 0xec, 0x07, 0x80, 0x44, 0x7d, 0xe0, 0x22, 0x00, 0x00,
        0x03, 0x00, 0x02, 0x00, 0x00, 0x03, 0x00, 0x65, 0xd1, 0x40, 0x01, 0xe8, 0x48, 0x00, 0x3d,
        0x09, 0x3e, 0xf7, 0x06, 0x8a, 0x00, 0x0f, 0x42, 0x40, 0x01, 0xe8, 0x49, 0xf7, 0xb8, 0x14,
    ];
