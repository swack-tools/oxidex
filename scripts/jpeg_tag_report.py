...
def main():
    ...
    md = ["# JPEG Tag Support\n",
          ...
          "ExifTool writes a sample into a clean JPEG and `oxidex -j -e` must "
          "read it back; writability additionally requires `oxidex -KEY=VALUE` "
          "to round-trip through both OxiDex and ExifTool.\n",
          "Only tags OxiDex can **read** from JPEG are listed here (including "
          "those whose value formatting differs from ExifTool). The full "
          "classification of all tested tags — including unsupported and "
          "broken ones — is in the [JPEG Tag Matrix](/reference/jpeg-tag-matrix). "
          "See also [ExifTool Coverage](/reference/tag-coverage-analysis) for "
          "the tag-database view and the "
          "[Compatibility overview](/reference/comparison/) for fixture-based "
    ...
    md = ["---", "outline: 2", "---\n",
          ...
          "(OxiDex writes → both read back), and known bugs with root causes. "
          "Readable tags with their working write keys are summarized in "
