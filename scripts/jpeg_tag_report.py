...
def main():
    ...
    md = ["# JPEG Tag Support: OxiDex ↔ ExifTool Mapping\n",
          "Empirically verified with `scripts/jpeg_tag_matrix.py`: for each tag, "
          "exiftool writes a sample into a clean JPEG and `oxidex -j -e` must read "
          "it back; writability additionally requires `oxidex -KEY=VALUE` to "
          "round-trip through both oxidex and exiftool.\n",
          "Only tags OxiDex can **read** from JPEG are listed here (including "
          "those whose value formatting differs from ExifTool). The full matrix "
          "including unsupported and broken tags is in "
          "[jpeg-tag-matrix.md](jpeg-tag-matrix.md).\n"]
    ...
    # ------------------------------------------------------- full matrix doc
    md = ["# JPEG Tag Matrix: Full ExifTool Tag Classification\n",
          "Every ExifTool-writable JPEG tag, classified by empirical test "
          "(`scripts/jpeg_tag_matrix.py`, exiftool 13.55, oxidex "
