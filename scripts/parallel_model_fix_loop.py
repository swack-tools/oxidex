...
# (moved there so there is exactly one fleet config file), the same file
# --config already points at for [worker]/[reviewer]/[publish] -- so there
# is no separate squad-manifest default here; every function below that
# used to take a squad-manifest path now takes args.config /
# DEFAULT_CONFIG_PATH (imported above from model_fix_loop) directly.
...
def real_build_attribution(cache_dir, config_path, out_path, perl_lib=None, repo_root=REPO_ROOT):
    ...
    module_to_squad, squad_names = attribute_gaps.load_squads(config_path)
...
def squad_worker_formats(squad, attribution, config_path):
    ...
    ordinarily be non-empty -- falling back to config.toml's advisory
    [squads.*] "formats" list (squad_merge_loop.squad_formats) when
    attribution has nothing recorded for this squad yet. Empty means
    "nothing to dispatch this squad this round" -- the caller skips it
    rather than spawning a worker with no format to work.
    """
    squads = (attribution or {}).get("squads") or {}
    ...
    if live:
        return list(live)
    try:
        return _sml().squad_formats(config_path, squad)
    except (OSError, ValueError):
        return []
...
def run_squad_round(args, config_path, build_attribution_fn=None, ensure_staging_branch_fn=None):
    ...
    build_attribution_fn = build_attribution_fn or (
        lambda cache_dir: real_build_attribution(cache_dir, config_path, args.gap_attribution_path)
    )
    ...
    for squad in sorted(allocation):
        formats = squad_worker_formats(squad, attribution, config_path)
        if not formats:
...
def auto_publish_round(*, repo_root=REPO_ROOT, cache_dir, home=None, config_path=None,
