...
#: Only these subjects are fleet fix work. Everything else on a worker
#: branch is sync/merge noise that is already on main by definition.
FIX_SUBJECT_HINTS = ("fix(", "feat(")
...
def main():
    ...
    for sha, source in candidates:
        ...
        if not any(h in meta["subject"] for h in FIX_SUBJECT_HINTS):
            continue
        pid = patch_id(repo, sha)
        if not pid:
            continue
