...
- **Three classes of tags with no ExifTool counterpart are hidden by default** (`a9072ba6`): hex-fallback names for unknown tags (such as `IFD0:0xF999`), JPEG SOF diagnostics, and ZIP per-entry forensics. `--extended-output` shows them. Other OxiDex-only tags, such as executable hardening summaries and `OOXML:` properties, are still printed by default.
...
- **The write APIs refuse any change they would not make.** `write_metadata`, `Metadata::save`/`write_to`, `CopyBuilder::execute`, `modify_tag`, `remove_tag`, `copy_metadata` and `shift_metadata_dates` now resolve every requested tag the way the CLI's `-TAG=VALUE` does and return the new `ExifToolError::TagsNotWritten` (naming each key) for a key the file's writer would drop -- `XMP:Title` in a JPEG, TIFF or PNG, `File:Comment`, most `IFD1:` keys, an ungrouped name that does not resolve -- with the file byte-identical. They used to return `Ok(())` having written nothing for those keys. `Ok` now means every change was applied, proven by reading the file back. An exhaustive `match` on `ExifToolError` must handle the new variant. `write_metadata` deletes a row only when the map was read from the same file and the caller removed it; a map built from scratch or read from another file only sets (ExifTool's SetNewValue model). `copy_metadata(src, dest, None)` is best-effort (every writable tag copied, the rest reported by `copy_metadata_report`), while a named tag that cannot be copied is refused. `write_metadata`, `modify_tag`, `remove_tag`, `copy_metadata`, `clear_all_metadata`, `Metadata::save`/`write_to` and `CopyBuilder::execute` return `WriteOutcome` (`Updated` / `Unchanged`, ExifTool WriteInfo's 1 / 2) instead of `()`. `ExifToolError`, `WriteOutcome`, `TagChange`, `ParseStatus`, `DiagnosticKind`, `TagNotWritten` and `CopyReport` are `#[non_exhaustive]`.
- **`-TagsFromFile` and `copy_metadata` copy by name, as ExifTool does** (#957). Each source tag goes to its name's preferred group and to every group the destination already carries it in, with the value as ExifTool prints it converted back as `-TAG=VALUE` converts it -- a JPEG's `File:ImageWidth` into XMP-tiff, `Make` into a PNG's text chunk and its eXIf IFD0, a Canon maker note's `MeteringMode` into ExifIFD. Where 13.59 writes and OxiDex cannot (XMP into a JPEG, a maker note block), `CopyReport::uncopied_tags` / `uncopied_groups` now name the family-1 group and tag (`XMP-tiff:ImageWidth`, `XMP-dc`), not the source's family-0 group (`XMP`), and the CLI warns with those groups. `CopyReport::requested` counts the names set.
...
- **Optional Magika AI-Powered File Detection** - Google's deep learning model for enhanced file type identification
  ...
  - **Measurements**: No beta-bound accuracy or performance receipt is published here
