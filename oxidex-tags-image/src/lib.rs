...
// (family, numeric tag ID) -> tag name, generated by build.rs from the same
// YAML as the database below. Reading it decodes nothing and allocates nothing.
include!(concat!(env!("OUT_DIR"), "/reverse_ids.rs"));
...
pub static IMAGE_TAGS: LazyLock<TagDatabase> = LazyLock::new(|| {
    bincode::serde::decode_from_slice(IMAGE_TAGS_BIN, bincode::config::legacy())
        .expect("Failed to deserialize pre-compiled image tags binary data")
...
mod tests {
    ...
    fn test_image_tags_loads() {
        ...
        validate_database(&IMAGE_TAGS).expect("Image tags should satisfy shared validation");
    }
}
