...
| Generated-table verification | Run the canonical aggregate recipe below under the exclusive lock after the setup probes pass. It checks committed declarations and modeled SubDirectory arithmetic against the pinned source; it does not observe file extraction. Generated tables must be regenerated by their producer, never hand-edited. |
...
### Canonical generated-table verification

After the setup and capability probe above pass, run the aggregate at the exact
clean candidate under the exclusive lock. The recipe probes the configured
canonical Perl/tree again, binds every native consumer to the resulting
receipt, creates a unique retained run directory, and propagates any comparison
failure. Use the no-argument recipe so `.exiftool-version` remains the only
release selector.
...
test "$(git rev-parse 'HEAD^{commit}')" = "$PARITY_SHA"
test -z "$(git status --porcelain)"
PARITY_TABLE_LOG="$PARITY_EVIDENCE/verify-tables.log"
python3 "$PARITY_LOCK" "$PARITY_TABLE_LOG" -- just verify-tables
test "$(git rev-parse 'HEAD^{commit}')" = "$PARITY_SHA"
test -z "$(git status --porcelain)"
jq -e 'select(.event == "end" and .exit == 0)' \
  "$PARITY_TABLE_LOG.status.jsonl" > "$PARITY_EVIDENCE/verify-tables-lock-end.json"
PARITY_TABLE_RUN=$(sed -n \
  's/^verify-tables evidence retained at //p' "$PARITY_TABLE_LOG" | tail -n 1)
test -n "$PARITY_TABLE_RUN"
case "$PARITY_TABLE_RUN" in
  "$CARGO_TARGET_DIR"/verify-tables-runs/run.*) ;;
  *) printf 'unexpected verify-tables evidence path: %s\n' "$PARITY_TABLE_RUN" >&2; exit 1 ;;
esac
test -s "$PARITY_TABLE_RUN/release-oracle.json"
test -s "$PARITY_TABLE_RUN/serial-verify-$PARITY_PIN.json"
jq -e --arg pin "$PARITY_PIN" --arg perl "$PARITY_PERL" \
  --arg tree "$PARITY_ET_TREE" \
  '.exiftool_version == $pin and .perl_path == $perl and .tree_path == $tree' \
  "$PARITY_TABLE_RUN/release-oracle.json" \
  > "$PARITY_EVIDENCE/verify-tables-oracle-identity.json"
shasum -a 256 "$PARITY_TABLE_LOG" \
  "$PARITY_TABLE_LOG.status.jsonl" \
  "$PARITY_TABLE_RUN/release-oracle.json" \
  "$PARITY_TABLE_RUN/serial-verify-$PARITY_PIN.json" \
  > "$PARITY_EVIDENCE/verify-tables.sha256"
```

A zero exit proves the committed binary/keyed/IFD declarations agree with the
pinned source checks performed by `verify.py`, the serial declaration census
agrees with its fresh source dump, and every modeled SubDirectory edge's
generated Start/Base arithmetic agrees with native Perl on the verifier's probe
set. The serial result is declaration verification, not native serial processor
execution. None of these checks demonstrates observed extraction from files;
use conformance and authenticated reads for that claim. Keep the catalog
ratchet separate: it cannot substitute for this aggregate or for runtime
measurement.

The receipt applies only to the exact `PARITY_SHA` checked before and after the
run. A later documentation or source commit does not inherit that measurement;
rerun before making a generated-table claim about the later SHA.
