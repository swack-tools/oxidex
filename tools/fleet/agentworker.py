...
2. Resolve conflicts on their merits, with these hard rules:
   - `src/exiftool_tables/binary_tables.rs` is GENERATED: always take the tip's version verbatim (`git checkout --theirs` semantics depend on merge direction -- VERIFY the result matches the tip by content: `git diff {tip_sha} -- src/exiftool_tables/binary_tables.rs` must be empty). Never hand-edit it, never invent enum variants. Regeneration only happens on the i7 host; if this branch changed the GENERATOR (tools/exiftool-tables/*.py), note in the commit message that regen on the i7 is still required before this branch can pass verify-tables.
