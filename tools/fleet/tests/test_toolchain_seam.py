...
WHAT THIS FILE PINS.
  ...
  6. `TestTheRefusalCodeIsItsOwn` -- BLOCKER 2, found porting this fix
     onto the post-split tree. As originally written the refusal
     returned rc 6, which `run_daemon` already used for "hub
     persistently unusable" -- a condition a supervisor SHOULD restart,
     where a toolchain mismatch is permanent until a human fixes the
     install. Nothing pinned it (classes 1-5 assert no rc at all), so
     the collision was silent. The refusal is rc 7 and that class drives
     both conditions through the real `fleetd.main` and requires them to
     differ.
...
# ------------------------------------------------------------------ #
# BLOCKER 2 -- the refusal code must not be the hub-unusable code
# ------------------------------------------------------------------ #


class TestTheRefusalCodeIsItsOwn(HermeticCase):
    """The port's own defect, fenced.

    As `dbf0ede0` wrote it, the L1 refusal returned **6** -- the code
    `run_daemon` already used, and still uses, for "a persistently
    unusable hub", which `tools/fleet/tests/test_fleetd.py` asserts in
    two places and `units/fleetd-wrapper.sh` restarts on. Two conditions
    under one code is not a cosmetic collision: a wedged hub SHOULD be
    restarted (the hub may come back), while a toolchain-id mismatch is
    a property of how the host is installed and cannot resolve without a
    human, so a supervisor that cannot tell them apart either restarts a
    permanently-refusing daemon forever or stops restarting the one case
    restarting fixes. The refusal is rc **7** here.

    Nothing pinned the refusal to 6 -- `test_toolchain_seam`'s other
    classes assert no rc at all -- which is exactly why the collision
    could be introduced silently, and is exactly why this class exists.

    Instrument: `fleetd.main` driven IN-PROCESS against a throwaway bare
    repo, with a real singleton claim, real `adopt_workers`, real signal
    handlers and the real `finally: singleton.release()`. Only
    `reconcile_once` is replaced, and only for the hub case. The gate
    command is a `_fake_gate_tree` whose resolver emits whichever
    platform_id the case needs, so the two paths differ in ONE input.
    """
    ...
    AGREE = object()  # sentinel: "emit whatever this runner will compute"

    def _run_main(self, platform_id, reconcile=None) -> int:
        """Drive `fleetd.main` with a gate whose resolver emits
        `platform_id`, or -- for `AGREE` -- exactly the id the runner
        itself derives.

        The gate tree is built INSIDE the patched environment, not
        outside it. `main` puts its object cache under `Path.home()`, so
        HOME is redirected into the tempdir, and
        `toolchain.compute_platform_id` resolves `rustc` off a PATH
        prefix built from that same HOME. Computing the "agreeing" id
        before the patch measured a different environment from the one
        the check runs in, which made the negative control fail as a
        mismatch -- the test-side instance of the very confusion this
        file exists to fence.
        """
        import unittest.mock as mock
        with mock.patch.dict(os.environ, {"HOME": str(self.tmp)}):
            wanted = (toolchain.compute_platform_id()
                      if platform_id is self.AGREE else platform_id)
            root = Path(tempfile.mkdtemp(dir=self.tmp))
            _fake_gate_tree(root, platform_id=wanted)
            argv = [
                "--hub", str(self.bare),
                "--repo-root", str(root),
            ...
            with mock.patch.object(fleetd, "reconcile_once", reconcile):
                return fleetd.main(argv)

    def test_a_toolchain_mismatch_exits_7(self):
        rc = self._run_main("DELIBERATELY-DIFFERENT")
        self.assertEqual(
            rc, 7,
            "the L1 refusal must have its own exit code -- 6 is the "
            "hub-unusable code the supervisor restarts on",
        )

    def test_a_wedged_hub_still_exits_6(self):
        """The inherited contract, unchanged. `test_fleetd.py` asserts
        this in two places and `run_daemon`'s docstring documents it;
        the L1 refusal must not have moved it."""
        ...
        def always_dead(*_a, **_kw):
            calls.append(1)
            raise HubError("simulated: ls-remote refs/fleet/desired failed")

        rc = self._run_main(self.AGREE, reconcile=always_dead)
        self.assertEqual(calls, [1], "the toolchain check refused before the hub step")
        self.assertEqual(rc, 6)

    def test_the_two_conditions_never_share_a_code(self):
        """The point of the two above, stated as one assertion so a
        future edit that collapses them fails HERE with the reason
        rather than somewhere downstream with a puzzle."""
        from fleetlib import HubError

        mismatch_rc = self._run_main("DELIBERATELY-DIFFERENT")

        def always_dead(*_a, **_kw):
            raise HubError("simulated: hub unusable")

        hub_rc = self._run_main(self.AGREE, reconcile=always_dead)

        self.assertNotEqual(
            mismatch_rc, hub_rc,
            "a toolchain mismatch and a wedged hub exit with the same code again. "
            "They need opposite supervisor behaviour: the hub may come back, the "
            "toolchain will not until a human fixes the install, so one code for "
            "both means either a hot restart loop or a hub outage nobody retries",
        )
        self.assertEqual((mismatch_rc, hub_rc), (7, 6))

    def test_the_rc_contract_is_documented_where_the_supervisor_reads_it(self):
        """`units/fleetd-wrapper.sh` is the file an operator opens to
        learn what an exit code means. A code the daemon can return and
        the contract does not mention is how the collision above got
        written in the first place."""
        wrapper = (FLEET_DIR / "units" / "fleetd-wrapper.sh").read_text(encoding="utf-8")
        self.assertIn("rc=7", wrapper,
                      "fleetd-wrapper.sh's rc contract does not mention rc 7")
        runner_doc = __import__("keel.runner", fromlist=["runner"]).run_daemon.__doc__
        self.assertIn("rc 7", runner_doc,
                      "run_daemon's docstring documents 0/3/4/5/6 but not 7")
