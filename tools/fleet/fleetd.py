...
def live_pgids() -> set:
    ...
    try:
        out = subprocess.run(
            ["ps", "-eo", "pgid="], capture_output=True, text=True, errors="replace", timeout=10
        ).stdout
...
def reconcile_once(
    ...
    hb = {
        "gates_running": len(workers),
        "agents_running": 0,  # agent workers are a later phase; honest zero
