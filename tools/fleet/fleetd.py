    ...
    if not desired_readable:
        ...
        if deficit > 0:
            ...
            if reason is not None:
                ...
                except workqueue.QueueError as e:
                    # B1 (Stage 1 integration review): `compute()` raises
                    # QueueError -- not HubError -- whenever the tip ref
                    # itself is missing on `hub` (e.g. a fleetd pointed via
                    # `--hub <state-repo>` at a state repo that carries no
                    # `refs/heads/*` at all, only `refs/fleet/*`). Before
                    # this guard that propagated straight out of
                    # `reconcile_once`, ahead of every hub-read guard above,
                    # and crashed the daemon before its first heartbeat.
                    # Degraded exactly like the HUB READS block: nothing
                    # starts this loop, the reason is on the record, the
                    # daemon lives to retry next loop.
                    q = None
                    res.refused.append(("queue-error", str(e)))
                ...
                if q is not None:
                    ...
                    if not candidates:
                        ...
                        # "queue-error" (the queue itself couldn't compute)
