...
from setnewvalue_addressing import (RUNTIME_STATUS, Addressing, _owned_names,
                                    ...
                                    _observed_candidates, native_candidate)
...
def generate(document: Mapping[str, Any], observations: Mapping[str, Any], *,
    ...
    prelude = (
        ...
        "pub(crate) struct StaticNativeLookupCandidate { pub name: &'static str, pub row_index: Option<usize>, pub source_identity_present: bool, pub module: Option<&'static str>, pub table: Option<&'static str>, pub full_name: Option<&'static str>, pub raw_id: &'static str, pub writable: Option<&'static str>, pub permanent: bool, pub write_group: Option<&'static str>, pub groups: &'static [StaticNativeLookupFamily] }\n"
    ...
    try:
        ...
        for name in sorted(addressing.owned_names):
            ...
            for candidate in candidates:
                ...
                lookup.append((name, index, identity is not None, _families(groups), native))
    ...
    chunks.extend(
        "    StaticNativeLookupCandidate { name: %s, row_index: %s, source_identity_present: %s, module: %s, table: %s, full_name: %s, raw_id: %s, writable: %s, permanent: %s, write_group: %s, groups: &[%s] },\n" % (
            rust_string(name), "Some(%d)" % index if index is not None else "None", "true" if source_identity_present else "false",
            _opt_text(native.module), _opt_text(native.table), _opt_text(native.full_name), rust_string(native.raw_id), _opt_text(native.writable), "true" if native.permanent else "false", _opt_text(native.write_group),
            ", ".join("StaticNativeLookupFamily { family: %d, value: %s }" % (family, rust_string(value)) for family, value in families))
        for name, index, source_identity_present, families, native in lookup)
    chunks.append("];\n")
