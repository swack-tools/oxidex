...
def generate(document: Mapping[str, Any], observations: Mapping[str, Any], *,
    ...
    prelude = (
        ...
        "pub(crate) struct StaticNativeLookupCandidate { pub name: &'static str, pub row_index: Option<usize>, pub group0: Option<&'static str>, pub group1: Option<&'static str>, pub group2: Option<&'static str> }\n"
    ...
    try:
        ...
        for name in sorted(addressing.owned_names):
            ...
            for candidate in candidates:
                ...
                lookup.append((name, index, groups.get("0"), groups.get("1"), groups.get("2")))
    ...
    chunks.extend(
        "    StaticNativeLookupCandidate { name: %s, row_index: %s, group0: %s, group1: %s, group2: %s },\n" % (
            rust_string(name), "Some(%d)" % index if index is not None else "None",
            _opt_text(group0), _opt_text(group1), _opt_text(group2))
        for name, index, group0, group1, group2 in lookup)
    chunks.append("];\n")
