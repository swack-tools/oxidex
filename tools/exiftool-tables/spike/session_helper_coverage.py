...
# Decode/Encode are not here: their one refusal is a 2-/4-byte charset whose
# byte order comes from GetByteOrder() on a Session with no byte order,
# which a reader that has parsed a TIFF header never hands them.
...
def render(p, path):
    ins = p["instrument"]
    out = ["# Session + helper library coverage (Autogeneration v2, step 1)", "",
