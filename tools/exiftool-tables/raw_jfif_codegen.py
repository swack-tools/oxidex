...
class Recipe:
    ...
    byte_order: str
    fields: tuple[Field, ...]
    creation_skip_markers: tuple[int, ...]
    creation_wait_for_directories: tuple[str, ...]
    creation_timing: str
...
def compile_fact(fact: Any) -> Recipe:
    ...
    # The entire WriteJPEG grammar fixes creation before current-segment
    # dispatch. Read its complete creation guard operands after full consumption.
    guards=re.findall(r"^            \(\(\(\(\$markerName eq '([^']+)'\) \|\| \$dirCount\{'([^']+)'\}\) \|\| \$dirCount\{'([^']+)'\}\) and \(last\)\);$",
        funcs['WriteJPEG']['body'],re.MULTILINE)
    if len(guards)!=1: raise Refused('unsupported native creation guard')
    skip_name,*wait_for_directories=guards[0]
    marker_names=mapping(fact.get('marker_names'),'JPEG marker names')
    source(marker_names,closure,'JPEG marker names')
    if marker_names.get('binding')!='Image::ExifTool::jpegMarker': raise Refused('JPEG marker name binding differs')
    names=mapping(marker_names.get('entries'),'JPEG marker name map')
    if names != contract['marker_names']: raise Refused('JPEG marker name map has unsupported control changes')
    def marker_name(code):
        direct=names.get(str(code))
        if direct: return direct
        group=names.get(str(code & 0xf0))
        match=re.fullmatch(r'([A-Z]+)\d+',group) if group else None
    ...
    return Recipe(marker,(dispatch['signature'].encode('ascii')+b'\0').hex(),skip,dispatch['order'],tuple(fields),skip_markers,wait_for_directories,'BeforeCurrentSegment')
...
use crate::writers::raw_segment_properties::{RawByteOrder, RawCreationTiming, RawField, RawSegmentRecipe};
...
'''+f'''    source_core_sha256: {json.dumps(fact['loaded_closure']['Image/ExifTool.pm'])},
    ...
    creation_skip_markers: &{list(recipe.creation_skip_markers)},
    creation_wait_for_directories: &[{', '.join(json.dumps(d) for d in recipe.creation_wait_for_directories)}],
    creation_timing: RawCreationTiming::{recipe.creation_timing},
    fields: &[\n{rows}\n    ],
}};
'''
