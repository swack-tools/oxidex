...
use oxidex::parsers::video::parse_mts_metadata;
...
fn test_mts_metadata_parity_with_exiftool() {
    let test_file = "test_data/video/sample.mts";
    ...
    // ("H264:ImageWidth"), and a plain `-json` emits bare tag names, so every
    ...
    // Run OxiDex
    let reader = BufferedReader::new(Path::new(test_file)).expect("Failed to open MTS file");
    let oxidex_metadata = parse_mts_metadata(&reader).expect("Failed to parse MTS file");

    // Compare key tags
    let tags_to_compare = ["H264:ImageWidth", "H264:ImageHeight"];
