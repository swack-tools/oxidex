...
use oxidex::parsers::video::parse_webm_metadata;
...
fn test_webm_metadata_parity_with_exiftool() {
    let test_file = "test_data/video/sample.webm";
    ...
    // Run OxiDex
    let reader = BufferedReader::new(Path::new(test_file)).expect("Failed to open WebM file");
    let oxidex_metadata = parse_webm_metadata(&reader).expect("Failed to parse WebM file");
