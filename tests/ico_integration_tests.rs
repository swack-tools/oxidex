mod common;
#[path = "common/fixtures.rs"]
mod fixtures;

use common::TestReader;
use oxidex::io::buffered_reader::BufferedReader;
use oxidex::parsers::image::ico::parse_ico_metadata;

#[test]
#[ignore = "requires the pinned ExifTool fixture cache"]
fn extracts_all_ico_directory_entry_fields_reported_by_pinned_exiftool() {
    let path =
        fixtures::pinned_t_images_fixture_path("ICO.ico").expect("open configured ICO fixture");
    let reader = BufferedReader::new(&path).expect("open ICO fixture");
    let metadata = parse_ico_metadata(&reader).expect("parse ICO fixture");

    assert_eq!(metadata.get_string("File:ImageWidth"), Some("1"));
    assert_eq!(metadata.get_string("File:ImageHeight"), Some("1"));
    assert_eq!(metadata.get_string("File:NumColors"), Some("2"));
    assert_eq!(metadata.get_string("File:ColorPlanes"), Some("1"));
    assert_eq!(metadata.get_string("File:BitsPerPixel"), Some("1"));
    assert_eq!(metadata.get_string("File:ImageLength"), Some("56"));
}

#[test]
fn does_not_treat_cursor_hotspot_y_as_bits_per_pixel() {
    let reader = TestReader::from_slice(&[
        0x00, 0x00, 0x02, 0x00, // reserved, cursor type
        0x01, 0x00, // one directory entry
        0x10, 0x10, 0x00, 0x00, // width, height, colors, reserved
        0x03, 0x00, // hotspot X
        0x07, 0x00, // hotspot Y (not bits per pixel)
        0x00, 0x00, 0x00, 0x00, // image length
        0x00, 0x00, 0x00, 0x00, // image offset
    ]);

    let metadata = parse_ico_metadata(&reader).expect("parse synthetic CUR");

    assert!(!metadata.contains_key("File:BitsPerPixel"));
}
